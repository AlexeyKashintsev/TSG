...
, sum(t2.sal_end)- (sum(t2.sal_begin)+sum(t2.sal_full_calc)+ sum(t2.sal_recalc)- sum(t2.sal_payments)) AS sal_penalties_cur, 
sum(t2.sal_recalc) AS sal_recalc 
From grp_lc_group t1
 Inner Join per_saldo_flat t2 on t1.lc_id = t2.lc_id
 Inner Join #DateGroupView q on q.per_date_id = t2.date_id
